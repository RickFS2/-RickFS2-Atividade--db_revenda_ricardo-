...
insert into pedidos (n_pedido, endereco_id, usuario_id) values 
('100', 1, 1),
('200', 2, 2),
('300', 3, 3),
('400', 4, 4),
...
EXPLAIN SELECT * FROM usuario WHERE nome LIKE 'A%';

--  Crie um index para a coluna que utilizou no filtro acima;

CREATE INDEX idx_usuario_nome ON usuario(nome);

-- Refaça a consulta e execute o explain novamente. Tire um novo print (anexe neste atividade) e compare com o anterior. Aponte as diferenças.

EXPLAIN SELECT * FROM usuario WHERE nome LIKE 'A%';

-- Altere uma coluna de varchar para int, avalie o retorno, inclusive se for erro;

ALTER TABLE produto ALTER COLUMN quantidade TYPE int USING quantidade::int;

--Altere uma coluna de int para varchar avalie o retorno, inclusive se for erro;

ALTER TABLE usuario ALTER COLUMN id TYPE varchar USING id::varchar;

--Crie um usuário com seu nome e dê todas as permissões de acesso para todas as tabelas;

CREATE USER saldanha WITH PASSWORD '2222';
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO saldanha;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO saldanha;

--Crie um usuário para seu colega apenas com permissão de select em uma das tabelas;

CREATE USER saldanha WITH PASSWORD '2222';
GRANT SELECT ON usuario TO saldanha;
...
--De volta no seu usuário, crie 12 consultas, sendo 3 consultas semelhantes (somente com alteração do tipo de join: inner, left e right). 
--Ou seja, são 4 consultas diferentes, sendo que cada consulta terá 3 versões, uma com cada tipo de join;

-- INNER 1
SELECT p.id, p.n_pedido, u.nome FROM pedidos p INNER JOIN usuario u ON p.usuario_id = u.id;

-- LEFT 1
SELECT p.id, p.n_pedido, u.nome FROM pedidos p LEFT JOIN usuario u ON p.usuario_id = u.id;

-- RIGHT 1
SELECT p.id, p.n_pedido, u.nome FROM pedidos p RIGHT JOIN usuario u ON p.usuario_id = u.id;

-- INNER 2
SELECT pg.id, pg.pix, p.n_pedido FROM pagamento pg INNER JOIN pedidos p ON pg.pedidos_id = p.id;

-- LEFT 2
SELECT pg.id, pg.pix, p.n_pedido FROM pagamento pg LEFT JOIN pedidos p ON pg.pedidos_id = p.id;

-- RIGHT 2
SELECT pg.id, pg.pix, p.n_pedido FROM pagamento pg RIGHT JOIN pedidos p ON pg.pedidos_id = p.id;

-- INNER 3
SELECT pr.titulo, pr.preco, p.n_pedido FROM produto pr INNER JOIN pedidos p ON pr.id = p.id;

-- LEFT 3
SELECT pr.titulo, pr.preco, p.n_pedido FROM produto pr LEFT JOIN pedidos p ON pr.id = p.id;

-- RIGHT 3
SELECT pr.titulo, pr.preco, p.n_pedido FROM produto pr RIGHT JOIN pedidos p ON pr.id = p.id;

-- INNER 4
SELECT u.nome, e.rua, e.cidade FROM usuario u INNER JOIN usuario_endereco ue ON u.id = ue.usuario_id INNER JOIN endereco e ON ue.endereco_id = e.id;

-- LEFT 4
SELECT u.nome, e.rua, e.cidade FROM usuario u LEFT JOIN usuario_endereco ue ON u.id = ue.usuario_id LEFT JOIN endereco e ON ue.endereco_id = e.id;

-- RIGHT 4
SELECT u.nome, e.rua, e.cidade FROM usuario u RIGHT JOIN usuario_endereco ue ON u.id = ue.usuario_id RIGHT JOIN endereco e ON ue.endereco_id = e.id;



--Atualize vários registros com colunas NULL;

UPDATE usuario SET e_mail = NULL WHERE id IN (1, 2, 3, 4, 5);
